...
2. `useProductPageNumber` через `findProductPage` листает `GET /api/products/` страницами по `CATALOG_PAGE_SIZE` с дефолтной сортировкой и без фильтров, пока не встретит slug. `MAX_SCANNED_PAGES` страхует от бесконечного `next`.
